...
Napi::Value MyLanguageModelOptions::GetContentFilterOptions(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        // Create a new MyContentFilterOptions wrapper with the existing WinRT object
...
Napi::Value MyContentFilterOptions::GetImageMaxAllowedSeverityLevel(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        // Create a new MyImageContentFilterSeverity wrapper with the existing WinRT object
...
Napi::Value MyContentFilterOptions::GetPromptMaxAllowedSeverityLevel(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        // Create a new MyTextContentFilterSeverity wrapper with the existing WinRT object
...
MyImageContentFilterSeverity::MyImageContentFilterSeverity(const Napi::CallbackInfo& info) : Napi::ObjectWrap<MyImageContentFilterSeverity>(info) {
    if (info.Length() == 1 && info[0].IsExternal()) {
        // Create from existing ImageContentFilterSeverity
        auto external = info[0].As<Napi::External<ImageContentFilterSeverity>>();
        m_options = *external.Data();
    } else if (info.Length() == 1 && info[0].IsNumber()) {
        // Create with SeverityLevel parameter
        ...
        // Create default ImageContentFilterSeverity
...
MyTextContentFilterSeverity::MyTextContentFilterSeverity(const Napi::CallbackInfo& info) : Napi::ObjectWrap<MyTextContentFilterSeverity>(info) {
    if (info.Length() == 1 && info[0].IsExternal()) {
        // Create from existing TextContentFilterSeverity
        ...
        // Create default TextContentFilterSeverity
...
Napi::Value MyLanguageModel::MyEnsureReadyAsync(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        // Handle progress updates (double values)
